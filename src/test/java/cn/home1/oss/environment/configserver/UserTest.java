...
import cn.home1.oss.environment.configserver.util.HttpUtil;
import cn.home1.oss.environment.configserver.util.TestEnv;
...
public class UserTest {
  ...
  curl -i -u admin:admin_pass -X POST -H 'Content-Type: application/x-www-form-urlencoded;' \
   ...
   "http://oss-configserver.local:8888/config/users/configserver-test/"
  查询用户：
  curl -i -u admin:admin_pass -X GET "http://oss-configserver.local:8888/config/users/configserver-test/"
  删除用户：
  curl -i -u admin:admin_pass -X DELETE "http://oss-configserver.local:8888/config/users/configserver-test/"
  更新用户密码：
